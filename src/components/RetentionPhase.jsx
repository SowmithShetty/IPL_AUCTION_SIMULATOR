  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto z-10 relative">
      <motion.div
        ...
        className="mb-8 mt-4 border-l-4 border-emerald-500 pl-6 flex flex-col md:flex-row justify-between md:items-end gap-4"
      >
        <div>
          <div className="text-[10px] text-emerald-400 uppercase tracking-[0.3em] font-black mb-2">Pre-Auction Phase</div>
          <h2 className="text-3xl md:text-4xl font-black italic text-white uppercase tracking-tighter">{franchiseName} Retention</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 md:gap-8">
            ...
            {squad.map((player, index) => {
              ...
              return (
                <motion.div
                  ...
                  className={`relative p-5 rounded-2xl cursor-pointer transition-all backdrop-blur-md ${
                    isSelected
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      <h3 className="font-black italic tracking-tight text-xl text-white">
                        {player.flag} {player.name}
                      </h3>
                      <div className="flex gap-2 items-center mt-1">
                        <span className="text-[10px] bg-black/50 px-2 py-0.5 rounded text-zinc-300 font-mono border border-white/10">{player.role}</span>
                        {player.isOverseas && (
                          <span className="text-[9px] bg-rose-500/10 px-2 py-0.5 rounded text-rose-400 font-black border border-rose-500/20 uppercase tracking-wider">Overseas</span>
                        )}
                      </div>
                    </div>
                  </div>

                  <div className="text-[10px] text-zinc-500 uppercase font-bold tracking-wider mb-4 flex items-center gap-1.5">
                  ...
                  <div className="flex justify-between items-center text-[10px] font-mono text-zinc-400 border-t border-white/5 pt-3">
                    <span>MAT: <span className="text-white">{player.stats.matches}</span></span>
                    {player.role === 'BOWL' ? (
                      <span>WKTS: <span className="text-white">{player.stats.wickets}</span> | ECO: <span className="text-white">{player.stats.eco}</span></span>
                    ) : (
                      <span>RUNS: <span className="text-white">{player.stats.runs}</span> | SR: <span className="text-white">{player.stats.sr}</span></span>
                    )}
                  </div>
                </motion.div>
              );
        ...
        <motion.div
          ...
          className="bg-white/[0.03] backdrop-blur-xl border border-white/10 p-6 rounded-[2rem] rounded-tr-sm rounded-bl-sm h-fit sticky top-8"
        >
          <h3 className="text-lg font-black italic tracking-tighter uppercase mb-6 text-white border-b border-white/10 pb-4">
            Retention Slots
          </h3>

          <div className="space-y-3 mb-8">
            {[0, 1, 2, 3].map(slot => (
              <div key={slot} className="flex justify-between items-center p-3 rounded-xl bg-black/40 border border-white/5">
          ...
          <motion.button
            ...
            className="w-full py-4 rounded-xl font-black uppercase tracking-widest bg-emerald-500 hover:bg-emerald-400 text-black shadow-[0_0_20px_rgba(16,185,129,0.3)] transition-all"
          >
            🏏 Begin Auction
          </motion.button>
        </motion.div>
